        ...
        for (size_t i = 0; i < VTOL_NUM_ACTUATORS; ++i) {
            if (!std::isfinite(vals_inp(i))) {
                if (std::isnan(vals_inp(i))) {
                    vals_inv_trans(i) = (upper_bounds(i) - lower_bounds(i)) / 2.0;
                }
                else if (vals_inp(i) < 0.0) {
    ...
    void* opt_data)
{
    ...
    const float step_min = 0.0;
    const float step_max = 10.0;
    const float xtol = 1E-04;

    // Wolfe parameters
    const float wolfe_cons_1 = (wolfe_cons_1_inp) ? *wolfe_cons_1_inp : 1E-03; // tolerance on the Armijo sufficient decrease condition; sometimes labelled 'mu'.
    const float wolfe_cons_2 = (wolfe_cons_2_inp) ? *wolfe_cons_2_inp : 0.90;  // tolerance on the curvature condition; sometimes labelled 'eta'.
    ...
    float st_best = 0.0, f_best = f_init, dgrad_best = dgrad_init;
    float st_other = 0.0, f_other = f_init, dgrad_other = dgrad_init;
                ...
                bfgs_settings_t* settings_inp)
{
    ...
    = [opt_objfn] (const matrix::Vector<float, VTOL_NUM_ACTUATORS>& vals_inp, matrix::Vector<float, VTOL_NUM_ACTUATORS>* grad_out, void* opt_data) 
    -> float 
    {
    ...
    bfgs_settings_t* settings_inp)
{
        ...
        -> float
    {
        ...
        matrix::Vector<float, n_vals> vals_inv_trans = inv_transform(vals_inp, lower_bounds, upper_bounds);
        float eps_flt = EPSILON_FLOAT;
        float ret;
        
        if (grad_out) {
            matrix::Vector<float, VTOL_NUM_ACTUATORS> grad_obj = *grad_out;
            ...
            for (size_t i = 0; i < n_vals; ++i) {
                jacob_matrix(i,i) = std::exp(vals_inp(i)) * (2*eps_flt + upper_bounds(i) - lower_bounds(i)) \
                                / (std::exp(2 * vals_inp(i)) + 2*std::exp(vals_inp(i)) + 1);
            }
            matrix::Vector<float, n_vals> jacob_vec = jacob_matrix.diag();

            for (size_t i = 0; i < n_vals; ++i)
                (*grad_out)(i) = jacob_vec(i) * grad_obj(i);
        } else {
